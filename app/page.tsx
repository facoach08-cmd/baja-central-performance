...
export default function Home() {
  ...
  const checked = clients.filter(c => c.last_health_check).length;
  ...
  function startEdit(client?: Client) {
    ...
    if (client) {
      ...
      setEditingId(null);
      setDraft({ name: "", segment: "", panel_name: "", status: "Online", monitor: "Cadastro manual", panel_url: "", feed_url: "", base_url: "", sort_order: clients.length + 1, feed_frequency_hours: 24, feed_notes: "" });
    }
  }
  ...
  async function saveClient() {
    ...
    const payload = {
      ...
      feed_frequency_hours: Math.max(1, Number(draft.feed_frequency_hours || 24)),
      feed_notes: draft.feed_notes || "",
  ...
  async function checkAll() {
    setCheckingAll(true);
    for (const client of clients) await checkHealth(client.id, true);
    await loadClients();
    setCheckingAll(false);
  }

  async function checkOne(clientId: string) {
    await checkHealth(clientId);
    await loadClients();
  }
  ...
  async function markFeed(client: Client) {
    ...
    if (error) alert(`Não foi possível registrar a alimentação: ${error.message}`);
  ...
  return <div className="min-h-screen bg-[#f4f7fb] text-[#123563]">
    <div className="grid min-h-screen md:grid-cols-[240px_1fr]">
      ...
      <main className="min-w-0 px-4 pb-10 md:px-7">
        ...
        {view === "Início" && <><section className="rounded-2xl bg-gradient-to-r from-[#0b3977] via-[#10519a] to-[#2e78c6] p-8 text-white shadow-sm"><h1 className="text-3xl font-bold md:text-4xl">Método antes de opinião.</h1><p className="mt-3 max-w-2xl text-blue-100">Centralize clientes, painéis, ferramentas e bases em um ambiente interno com dados persistidos no Supabase.</p></section><section className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Card label="Clientes monitorados" value={clients.length} note="Carteira cadastrada"/><Card label="Painéis online" value={`${online}/${clients.length}`} note="Status da última checagem"/><Card label="Bases atrasadas" value={overdueFeeds} note="Conforme frequência definida"/><Card label="Links pendentes" value={missingLinks} note="Alimentação ou base faltando"/></section><section className="mt-4 grid gap-4 xl:grid-cols-[1.35fr_.65fr]"><div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="mb-4 flex items-center justify-between"><h2 className="font-bold">Carteira monitorada</h2><button className="text-sm font-semibold text-blue-600" onClick={()=>setView("Painéis")}>Ver monitoramento →</button></div><ClientTable data={clients} canEdit={isAdmin} onEdit={startEdit} onOpen={open}/></div><div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="font-bold">Acessos rápidos</h2><div className="mt-4 grid gap-2">{clients.map(c=><div key={c.id} className="rounded-xl border border-slate-200 p-3"><div className="flex items-center justify-between gap-3"><div><strong>{c.name}</strong><span className="block text-xs text-slate-500">{c.panel_name}</span></div><button onClick={()=>open(c.panel_url,"painel")} className="rounded-lg bg-blue-50 px-3 py-2 text-xs font-semibold text-blue-700">Abrir</button></div><div className="mt-2 flex items-center justify-between"><FeedBadge client={c}/>{isAdmin && <button onClick={()=>markFeed(c)} className="text-xs font-semibold text-slate-500">Registrar alimentação</button>}</div></div>)}</div></div></section></>}

        {view === "Clientes" && <><PageTitle title="Clientes" subtitle="Cadastros e regras de atualização ficam persistidos no Supabase." action={isAdmin ? <button onClick={()=>startEdit()} className="rounded-xl bg-blue-600 px-4 py-3 text-sm font-semibold text-white">+ Novo cliente</button> : undefined}/><div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><ClientTable data={filtered} canEdit={isAdmin} onEdit={startEdit} onOpen={open}/></div></>}

        {view === "Painéis" && <><PageTitle title="Painéis e Ferramentas" subtitle="Disponibilidade técnica e atualização das bases em um só lugar." action={<button disabled={checkingAll} onClick={checkAll} className="rounded-xl bg-blue-600 px-4 py-3 text-sm font-semibold text-white disabled:opacity-50">{checkingAll ? "Verificando..." : "Atualizar todos os status"}</button>}/><div className="overflow-x-auto rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><table className="w-full min-w-[1400px] text-sm"><thead><tr className="bg-slate-50 text-left text-xs text-slate-500"><th className="p-3">Painel</th><th className="p-3">Cliente</th><th className="p-3">Status</th><th className="p-3">Última checagem</th><th className="p-3">Resposta</th><th className="p-3">Última alimentação</th><th className="p-3">Frequência</th><th className="p-3">Base</th><th className="p-3">Ações</th></tr></thead><tbody>{clients.map(c=><tr key={c.id} className="border-b border-slate-100"><td className="p-3 font-semibold">{c.panel_name}</td><td className="p-3">{c.name}</td><td className="p-3"><Badge status={c.status}/></td><td className="p-3 text-slate-500">{formatDate(c.last_health_check)}</td><td className="p-3 text-slate-500">{c.last_http_status ? `HTTP ${c.last_http_status}` : "—"}{c.last_latency_ms !== null ? <span className="block text-xs text-slate-400">{c.last_latency_ms} ms</span> : null}</td><td className="p-3"><FeedBadge client={c}/><div className="mt-1 text-xs text-slate-400">{formatDate(c.last_feed_at)}</div>{c.feed_notes && <div className="mt-1 max-w-[220px] text-xs text-slate-400">{c.feed_notes}</div>}</td><td className="p-3 text-slate-500">a cada {c.feed_frequency_hours}h</td><td className="p-3"><button onClick={()=>open(c.base_url,"base")} className={c.base_url?"text-blue-600 underline":"text-slate-400"}>{c.base_url?"Abrir":"Pendente"}</button></td><td className="p-3"><div className="flex gap-2"><button disabled={healthBusy===c.id || checkingAll} onClick={()=>checkOne(c.id)} className="rounded-lg border border-blue-200 px-3 py-2 text-xs font-semibold text-blue-700 disabled:opacity-50">{healthBusy===c.id?"Checando...":"Checar painel"}</button>{isAdmin && <button disabled={feedBusy===c.id} onClick={()=>markFeed(c)} className="rounded-lg border border-emerald-200 px-3 py-2 text-xs font-semibold text-emerald-700 disabled:opacity-50">{feedBusy===c.id?"Registrando...":"Registrar alimentação"}</button>}</div></td></tr>)}</tbody></table></div></>}

        {view === "Alertas" && <><PageTitle title="Alertas" subtitle="Painéis indisponíveis, bases atrasadas e cadastros incompletos."/><div className="grid gap-3">{clients.filter(c=>c.status !== "Online" || feedState(c).overdue || !c.feed_url || !c.base_url).map(c=><div key={c.id} className={`rounded-2xl border bg-white p-5 shadow-sm ${c.status === "Offline" || feedState(c).kind === "late" ? "border-red-200" : "border-amber-200"}`}><div className="flex flex-wrap items-center gap-3"><div className="font-bold">{c.name}</div><Badge status={c.status}/><FeedBadge client={c}/></div><div className="mt-2 text-sm text-slate-500">{c.status !== "Online" && `${c.monitor}. `}{feedState(c).overdue && `A última alimentação esperada é a cada ${c.feed_frequency_hours}h. `}{!c.feed_url && "Falta cadastrar a ferramenta de alimentação. "}{!c.base_url && "Falta cadastrar a base de dados."}</div></div>)}</div></>}
