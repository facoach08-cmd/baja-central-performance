...
export default function Home() {
  ...
  async function loadClients() {
    const { data, error } = await supabase.from("baja_central_clients").select("id,name,segment,panel_name,status,monitor,panel_url,feed_url,base_url,sort_order,last_health_check,last_http_status,last_latency_ms,last_feed_at").order("sort_order", { ascending: true });
  ...
  return <div className="min-h-screen bg-[#f4f7fb] text-[#123563]">
    <div className="grid min-h-screen md:grid-cols-[240px_1fr]">
      ...
      <main className="min-w-0 px-4 pb-10 md:px-7">
        ...
        {view === "Painéis" && <><PageTitle title="Painéis e Ferramentas" subtitle="Status técnico dos painéis com checagem direta do endereço publicado." action={<button disabled={checkingAll} onClick={checkAll} className="rounded-xl bg-blue-600 px-4 py-3 text-sm font-semibold text-white disabled:opacity-50">{checkingAll ? "Verificando..." : "Atualizar todos os status"}</button>}/><div className="overflow-x-auto rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><table className="w-full min-w-[1180px] text-sm"><thead><tr className="bg-slate-50 text-left text-xs text-slate-500"><th className="p-3">Painel</th><th className="p-3">Cliente</th><th className="p-3">Status</th><th className="p-3">Última checagem</th><th className="p-3">Resposta</th><th className="p-3">Monitoramento</th><th className="p-3">Painel</th><th className="p-3">Alimentação</th><th className="p-3">Base</th><th className="p-3">Ação</th></tr></thead><tbody>{clients.map(c=><tr key={c.id} className="border-b border-slate-100"><td className="p-3 font-semibold">{c.panel_name}</td><td className="p-3">{c.name}</td><td className="p-3"><Badge status={c.status}/></td><td className="p-3 text-slate-500">{formatDate(c.last_health_check)}</td><td className="p-3 text-slate-500">{c.last_http_status ? `HTTP ${c.last_http_status}` : "—"}{c.last_latency_ms !== null ? <span className="block text-xs text-slate-400">{c.last_latency_ms} ms</span> : null}</td><td className="max-w-[260px] p-3 text-slate-500">{c.monitor}</td><td className="p-3"><button onClick={()=>open(c.panel_url,"painel")} className="text-blue-600 underline">Abrir</button></td><td className="p-3"><button onClick={()=>open(c.feed_url,"alimentação")} className={c.feed_url?"text-blue-600 underline":"text-slate-400"}>{c.feed_url?"Abrir":"Pendente"}</button></td><td className="p-3"><button onClick={()=>open(c.base_url,"base")} className={c.base_url?"text-blue-600 underline":"text-slate-400"}>{c.base_url?"Abrir":"Pendente"}</button></td><td className="p-3"><button disabled={healthBusy===c.id || checkingAll} onClick={()=>checkOne(c.id)} className="rounded-lg border border-blue-200 px-3 py-2 text-xs font-semibold text-blue-700 disabled:opacity-50">{healthBusy===c.id?"Checando...":"Checar agora"}</button></td></tr>)}</tbody></table></div></>}

        {view === "Alertas" && <><PageTitle title="Alertas" subtitle="Pendências objetivas da Central."/><div className="grid gap-3">{clients.filter(c=>c.status !== "Online" || !c.feed_url || !c.base_url).map(c=><div key={c.id} className={`rounded-2xl border bg-white p-5 shadow-sm ${c.status === "Offline" ? "border-red-200" : "border-amber-200"}`}><div className="flex items-center gap-3"><div className="font-bold">{c.name}</div><Badge status={c.status}/></div><div className="mt-2 text-sm text-slate-500">{c.status !== "Online" && `${c.monitor}. `}{!c.feed_url && "Falta cadastrar a ferramenta de alimentação. "}{!c.base_url && "Falta cadastrar a base de dados."}</div></div>)}</div></>}
...
function ClientTable({ data, canEdit, onEdit, onOpen }: { data:Client[]; canEdit:boolean; onEdit:(c?:Client)=>void; onOpen:(u:string,l:string)=>void }) { return <div className="overflow-x-auto"><table className="w-full min-w-[900px] text-sm"><thead><tr className="bg-slate-50 text-left text-xs text-slate-500"><th className="p-3">Cliente</th><th className="p-3">Status</th><th className="p-3">Última checagem</th><th className="p-3">Monitoramento</th><th className="p-3">Ações</th></tr></thead><tbody>{data.map(c=><tr key={c.id} className="border-b border-slate-100 hover:bg-slate-50"><td className="p-3"><div className="font-bold">{c.name}</div><div className="text-xs text-slate-400">{c.panel_name}</div></td><td className="p-3"><Badge status={c.status}/></td><td className="p-3 text-slate-500">{formatDate(c.last_health_check)}</td><td className="p-3 text-slate-500">{c.monitor}</td><td className="p-3"><div className="flex flex-wrap gap-3"><button onClick={()=>onOpen(c.panel_url,"painel")} className="font-semibold text-blue-600">Abrir painel</button>{canEdit && <button onClick={()=>onEdit(c)} className="font-semibold text-[#0b3977]">Editar links</button>}</div></td></tr>)}</tbody></table></div>; }